...
class LLaDAModelLM(LLaDAPreTrainedModel):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    mask_index = (x == mask_id)
                    mask_index[:, :inputs_embeds.shape[1]] = False
                    ...
                    for token_id in [126081, 126080, 126346, 126347, mask_id]:
    ...
    @torch.no_grad()
    def generate_with_embeds_replace_noise(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
                                           cfg_scale=0., remasking='low_confidence', mask_id=126336, tokenizer=None,
                                           stopping_criteria=None, generation_suffix=None, **kwargs):
        '''
        Args:
            inputs_embeds: A tensor of shape (1, l, d).
            steps: Sampling steps, less than or equal to gen_length.
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if generation_suffix is not None and tokenizer is not None and len(generation_suffix) > 0:
                ...
                suffix_token_ids = torch.tensor(suffix_token_ids, dtype=torch.long,
                                                device=inputs_embeds.device).unsqueeze(0)  # (1, s)
                # Convert to embedding
                suffix_embeds = self.model.embed_tokens(suffix_token_ids)  # (1, s, d)
                suffix_len = suffix_embeds.shape[1]
            else:
            ...
            x_embeds = masked_embed.repeat(1, total_length, 1).to(
                inputs_embeds.device)  # shape (1, l + gen_length + suffix_len, d)
            x_embeds[:, :inputs_embeds.shape[1]] = inputs_embeds.clone()
            if suffix_embeds is not None:
                x_embeds[:, -suffix_len:] = suffix_embeds
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    all_steps_responses.append(x[:, inputs_embeds.shape[1]:inputs_embeds.shape[1] + gen_length].clone())
                    ...
                    if cfg_scale > 0.:
                        ...
                        un_mask = prompt_index.unsqueeze(-1).expand_as(
                            x_embeds)  # shape (1, l + gen_length + suffix_len, d)
                        un_embeds[un_mask] = masked_embed.repeat(x_embeds.shape[0], x_embeds.shape[1], 1)[
                    ...
                    # Add noise and get the most likely token
                    logits_with_noise = self.add_gumbel_noise(logits,
                                                              temperature=temperature)  # shape (1, l + gen_length + suffix_len, vocab_size)
                    x0 = torch.argmax(logits_with_noise, dim=-1)  # 1, l + gen_length + suffix_len

                    # Get confidence scores
                    if remasking == 'low_confidence':
                        p = F.softmax(logits.to(torch.float64),
                                      dim=-1)  # shape (1, l + gen_length + suffix_len, vocab_size)
                        x0_p = torch.squeeze(
                            torch.gather(p, dim=-1, index=torch.unsqueeze(x0, -1)),
                            -1)  # 1, l + gen_length + suffix_len represents the confidence of each x0
                    elif remasking == 'random':
                        x0_p = torch.rand((x0.shape[0], x0.shape[1]), device=x0.device)
                    else:
                        raise NotImplementedError(remasking)
                    ...
                    if stopping_criteria is not None:
                        ...
                        for stop_seq in stop_tokens:
                            ...
                            for start_idx in range(generated_part.size(0) - len(stop_seq) + 1):
                                if torch.all(
                                        generated_part[start_idx:start_idx + len(stop_seq)] == torch.tensor(stop_seq,
                                                                                                            device=x.device)):
                                    # Calculate the position of the currently found stop word
                                    current_position = inputs_embeds.shape[1] + start_idx
                                    # If it is the first time a stop word is found, or this stop word is earlier than the previously found one
            ...
            if found_stop_seq:
                if suffix_len > 0:
                    return torch.cat([
                        x[:, inputs_embeds.shape[1]:stop_position],
                        x[:, -suffix_len:]
                    ], dim=1), all_steps_responses
                else:
                    return x[:, inputs_embeds.shape[1]:stop_position], all_steps_responses
            else:
                if suffix_len > 0:
                    return torch.cat([
                        x[:, inputs_embeds.shape[1]:inputs_embeds.shape[1] + gen_length],
                    ...
                    return x[:, inputs_embeds.shape[1]:inputs_embeds.shape[1] + gen_length], all_steps_responses
